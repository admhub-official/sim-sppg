...
def remove_js_function(text: str, name: str, required: bool = True) -> str:
    ...
    if brace < 0:
        raise SystemExit(f'function brace missing: {name}')
...
app = replace_once(
    ...
    'registration mutation functions',
)

# setAuthMode now only manages the login container.
start = app.find('function setAuthMode(mode) {')
end = app.find('function showLogin() {', start)
if start < 0 or end < 0:
    raise SystemExit('setAuthMode block missing')
...
# Remove only the obsolete functions, preserving shared profile/autocomplete code.
for fn in [
    'showRegister', 'checkPasswordStrength', 'previewRegFoto',
    'doRegister', 'submitRegister', 'showOtpVerification',
    'doVerifyOtp', 'doResendOtp'
]:
    app = remove_js_function(app, fn)
...
# New menu is canonical in MENU_CONFIG, not injected by a runtime patch.
app = replace_once(
    app,
...
# Remove the later compatibility runtime for public registration.
app = app.replace(
...
if '  function installRegistrationRouting() {' in app:
    app = remove_between(app, '  function installRegistrationRouting() {', '  function ensureRoleMenus() {', 'runtime registration wrapper')
...
if "'register-user-v2': { createUserBySuperAdmin:1 }" not in app:
    raise SystemExit('admin user route missing')
...
if reg_start < 0 or otp_start < 0:
    raise SystemExit('register/otp markup missing')
html = html[:reg_start] + html[otp_start:]

otp_start = html.find('      <div id="otpForm"')
auth_end = html.find('    </section>\n  </main>\n</div><!-- /authOverlay -->', otp_start)
if otp_start < 0 or auth_end < 0:
    raise SystemExit('otp auth end missing')
html = html[:otp_start] + html[auth_end:]
...
html = html.replace(
    ...
    '  #authOverlay.auth-architecture {', 1,
)
...
if 'id="page-add-user"' not in html:
    raise SystemExit('add-user page missing')
index_path.write_text(html, encoding='utf-8')


# -----------------------------------------------------------------------------
# supplier-dropdown.js — remove the temporary runtime override.
# -----------------------------------------------------------------------------
...
print('auth/registration dead-code cleanup applied')
