# Deterministic patch for verifier TTD state preservation and cache bust.
from pathlib import Path

app_path = Path('app.js')
text = app_path.read_text(encoding='utf-8')

if "var verifTtdBase64Temp = '';" not in text:
    replacements = [
        (
...
old = '<script src="./app.js?v=20260720-approval-v2"></script>'
new = '<script src="./app.js?v=20260721-verifier-ttd-v3"></script>'
if old in index:
    index = index.replace(old, new, 1)
elif new not in index:
    raise SystemExit('Expected app.js cache-bust marker not found')
