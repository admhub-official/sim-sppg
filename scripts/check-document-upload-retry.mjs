...
const oldSession = context.window.uploadDocumentFiles([file('private.txt'), file('never-started.txt')]);
context.window.clearDocumentUploadQueue();
rejectRequest(new Error('offline'));
await oldSession;
assert.equal(host.innerHTML, '', 'logout clears file references and stale failures cannot restore them');
assert.equal(calls, 4, 'logout stops subsequent files in the old batch');
