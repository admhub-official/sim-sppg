...
declare
  ...
  v_count integer := 0;
begin
  if nullif(btrim(coalesce(p_transaksi_id, '')), '') is null then
    raise exception 'transaksi_id wajib diisi';
  end if;

  if not exists (
    select 1 from public."TRANSAKSI" where "ID" = p_transaksi_id
  ) then
    raise exception 'Transaksi tidak ditemukan';
  end if;

  if p_documents is null or jsonb_typeof(p_documents) <> 'array' then
    raise exception 'documents harus berupa array JSON';
  end if;
  ...
  loop
    ...
    if v_expected_bucket is null then
      raise exception 'Jenis dokumen tidak diizinkan: %', v_type;
    end if;

    if v_bucket is distinct from v_expected_bucket then
      raise exception 'Bucket tidak sesuai untuk jenis dokumen %', v_type;
    end if;

    if nullif(v_path, '') is null or upper(v_path) in ('-', 'FOTO', 'FILE') then
      raise exception 'storage_path tidak valid untuk jenis dokumen %', v_type;
    end if;
  ...
  return jsonb_build_object(
    ...
    'documents_written', v_count,
    'write_source', 'TRANSAKSI_DOCUMENTS'
  );
end;
$$;

revoke all on function public.save_transaction_documents_atomic(text, jsonb, text, text) from public;
revoke all on function public.save_transaction_documents_atomic(text, jsonb, text, text) from anon;
revoke all on function public.save_transaction_documents_atomic(text, jsonb, text, text) from authenticated;
