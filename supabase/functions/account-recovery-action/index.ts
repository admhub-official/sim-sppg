...
async function recoverPassword(data:any,ip:string){
  ...
  const q=await sb.auth.resetPasswordForEmail(email);
  if(q.error)console.error('recovery email error',q.error.message);
  return{success:true,message:GENERIC};
}
Deno.serve(async req=>{if(req.method==='OPTIONS')return new Response('ok',{headers:C});if(req.method==='GET')return out({status:'ok',service:'account-recovery-action',version:4,native:true,persistentRateLimit:true});if(req.method!=='POST')return out({error:'Method tidak didukung.'},405);if(Number(req.headers.get('content-length')||0)>16000)return out({error:'Payload terlalu besar.'},413);try{const body=await req.json();const fn=String(body?.function||'');if(fn!=='recoverPassword')return out({error:'Fungsi tidak diizinkan.'},404);const data=Array.isArray(body.parameters)?body.parameters[0]||{}:{};const ip=req.headers.get('x-forwarded-for')?.split(',')[0]?.trim()||'unknown';return out({result:await recoverPassword(data,ip)})}catch(e){console.error(e);return out({result:{success:true,message:GENERIC}})}});
