...
function recoveryRedirect(candidate:string){
  try{
...
async function recoverPassword(data:any,ip:string,origin:string,referer:string){
...
Deno.serve(async req=>{
  ...
  try{
    ...
    if(fn!=='recoverPassword')return out({error:'Fungsi tidak diizinkan.'},404);
    const data=Array.isArray(body.parameters)?body.parameters[0]||{}:{};
    const ip=req.headers.get('x-forwarded-for')?.split(',')[0]?.trim()||'unknown';
    const origin=req.headers.get('origin')||'';
    const referer=req.headers.get('referer')||'';
    return out({result:await recoverPassword(data,ip,origin,referer)});
  }catch(e){
    console.error(e);
    return out({result:{success:true,message:GENERIC}});
  }
});
