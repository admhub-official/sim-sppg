...
async function getBooleanSetting(key:string,fallback=false){
  const q=await sb.from('APP_SETTINGS').select('VALUE').eq('KEY',key).maybeSingle();
  if(q.error)throw q.error;
  if(!q.data)return fallback;
  return String(q.data.VALUE||'').toLowerCase()==='true';
}
async function setBooleanSetting(key:string,enabled:boolean,c:Caller){
  if(!['ADMIN','SUPER_ADMIN'].includes(c.role))throw new Error('Hanya ADMIN/SUPER ADMIN yang dapat mengubah pengaturan fitur.');
  const value=enabled?'true':'false';
  const existing=await sb.from('APP_SETTINGS').select('KEY').eq('KEY',key).maybeSingle();
  if(existing.error)throw existing.error;
  const q=existing.data
    ? await sb.from('APP_SETTINGS').update({VALUE:value}).eq('KEY',key)
    : await sb.from('APP_SETTINGS').insert({KEY:key,VALUE:value});
  if(q.error)throw q.error;
  await audit(c,'APP_SETTINGS',key,'TOGGLE_MODE',{enabled});
  return{success:true,enabled,message:'Pengaturan fitur berhasil diperbarui.'};
}
async function getUploadBuktiMode(_c:Caller){return{success:true,enabled:await getBooleanSetting('ALLOW_USER_UPLOAD_BUKTI',false)}}
