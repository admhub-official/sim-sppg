...
export const TABLE = { users: 'USERS', tx: 'TRANSAKSI', assignment: 'ADMIN_ASSIGNMENT', proofs: 'TRANSAKSI_PAYMENT_PROOFS', settings: 'APP_SETTINGS', audit: 'AUDIT LOG' };
...
export const normalizeStatus = (value: unknown) => { const x = text(value).toUpperCase().replace(/\s+/g, '_'); return x === 'LUNAS' ? 'SUDAH_DIBAYAR' : (x || 'BELUM_BAYAR'); };
