(function () {
  ...
  function install() {
    ...
    window.exportApprovalReportPDF = function (data, pageLabel) {
      ...
      var summaryRows = summary.map(function (group) {
      ...
      var html = '<!doctype html><html lang="id"><head><meta charset="utf-8"><title>' + esc(title) + '</title><style>' +
        ...
        summaryRows + '</tbody></table></body></html>';
    ...
    window.exportApprovalReportCSV = function (data) {
      ...
      summary.forEach(function (group) {
        add([group.supplier,group.account,group.count,group.complete,group.incomplete,Math.round(group.total)]);
      });
    ...
    window.exportApprovalSpreadsheet = function (data, format) {
      ...
      window._loadApprovalSpreadsheetLibrary().then(function () {
        ...
        var detailRows = [['No','Tanggal','Kode','SPPG','Nama Penginput','Email','Jenis Kategori','Nama Item','Supplier / Penjual','Rekening Penerima','Nama Bank','Nomor Rekening','Atas Nama','Nominal (Rp)','Status Pembayaran','Status Kelengkapan']];
        report.rows.forEach(function (row) {
        ...
        var summaryRows = [['Supplier / Penjual','Rekening Penerima','Jumlah Transaksi','Dokumen Lengkap','Dokumen Belum Lengkap','Total Nominal (Rp)']];
        summary.forEach(function (group) {
          summaryRows.push([group.supplier,group.account,group.count,group.complete,group.incomplete,Math.round(group.total)]);
        });
        window._appendApprovalSheet(workbook, 'Detail Approval', detailRows, [6,16,22,16,22,30,24,34,28,42,18,22,26,18,25,27], 1);
        window._appendApprovalSheet(workbook, 'Rekap Supplier', summaryRows, [30,44,18,20,24,22], 1);
