...
const AUTH_UI_SCRIPT = '<script src="/auth-ui-v2.js?v=20260715-3"></script>';
...
function removeUnstableAndLegacyInjections(html) {
  return html
    .replace(/\s*<script\b[^>]*src=["'][^"']*\/laporan-fix\.js(?:\?[^"']*)?["'][^>]*><\/script>\s*/gi, '\n')
    .replace(/\s*<script\b[^>]*src=["'][^"']*\/auth-responsive-fix\.js(?:\?[^"']*)?["'][^>]*><\/script>\s*/gi, '\n');
}
...
export default {
  async fetch(request, env) {
    ...
    let html = removeUnstableAndLegacyInjections(await response.text());
